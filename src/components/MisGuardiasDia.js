...
const MisGuardias = ({misguardias,date}) => {
    ...
    if (1) {
        const diaSelect = moment(date).format("YYYY-MM-DD")
        const x =misguardias.filter(misguardias => misguardias.date === diaSelect)
        if (x.length !== 0) {
            return (
                <>
                {misguardias.filter(misguardias => misguardias.date === diaSelect).map(
                    ...
                    <div className="text-center space-y-2 sm:text-left">
                        <div className="space-y-0.5">
                            <p className="text-lg text-black font-semibold">
                                No hay guardias para el día seleccionado.
                            </p>
                        </div>
                    </div>
                </div>
